...
async def list_alerts(
    ...
    return await container.alert_service.list_alerts(
...
async def update_alert_status(
    ...
    try:
        current_alert = (
            await container.alert_service.get_alert(GetAlertQuery(alert_id=alert_id))
        ).alert
        if body.linked_case_id is not None:
            await container.case_service.get_case(GetCaseQuery(case_id=body.linked_case_id))
        result = await container.alert_service.update_status(
            UpdateAlertStatusCommand(
                alert_id=alert_id,
                status=body.status,
...
async def create_case_from_alert(
    ...
    request.state.audit_resource_id = alert_id
    try:
        alert = (await container.alert_service.get_alert(GetAlertQuery(alert_id=alert_id))).alert
        ...
        case_command = await build_case_command_from_alert(alert, container)

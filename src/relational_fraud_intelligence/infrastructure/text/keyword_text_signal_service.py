...
class KeywordTextSignalService:
    _note_keywords: tuple[tuple[str, str, float, str], ...] = (
        ("synthetic identity", "synthetic identity", 0.94, "The note explicitly references synthetic identity behavior."),
        ("shared device", "device sharing", 0.88, "The note references a shared device used across identities."),
        ...
        ("credential", "credential compromise", 0.78, "The note mentions credential compromise or stuffing."),
    )

    _merchant_keywords: tuple[tuple[str, str, float, str], ...] = (
        ("gift card", "gift card liquidation", 0.89, "The merchant description indicates digital gift card resale."),
        ("instant resale", "rapid resale channel", 0.8, "The merchant supports fast liquidation."),
        ("cross-border payout", "cross-border payouts", 0.84, "The merchant operates in cross-border disbursement."),
    ...
    def score(self, command: ScoreTextSignalsCommand) -> ScoreTextSignalsResult:
        ...
        for merchant in command.scenario.merchants:
            normalized_description = f"{merchant.display_name} {merchant.description} {merchant.category}".lower()
